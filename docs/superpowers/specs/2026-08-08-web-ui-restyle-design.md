...
| `--gray-400` | `#adb5bd` | プレースホルダのみ（AA 未満のため文字色には他で使わない） |
| `--gray-500` | `#6b7280` | 補助テキスト・テーブル見出し |
...
本文サイズ（0.9375rem）で使う文字色は WCAG AA（4.5:1）を満たすこと。
この branch は以前にも AA 対応の修正を入れている（コミット `0cc91b5`）ので、そこに合わせる。

| 組み合わせ | 比 |
|---|---|
| `--gray-900` on `--gray-50`（本文） | 14.6:1 |
| `--gray-700` on `--gray-0`（ラベル） | 8.18:1 |
| `--gray-500` on `--gray-0`（補助テキスト・テーブル見出し） | 4.83:1 |
| `--gray-500` on `--gray-50` | 4.59:1 |
| `--danger` on `--gray-0`（500 ページ） | 5.46:1 |
| `--danger` on `--danger-soft`（エラー表示・バッジ） | 5.10:1 |

`--gray-400` は白背景で 2.08:1 しかなく AA を満たさない。**文字色としてはプレースホルダにのみ使う。**

期限切れ行の日時にも使いたくなるが、使ってはならない。「期限切れ」であるという事実は
バッジが伝えるので色だけに依存しないが、**日時の値そのものは他のどこにも出ていない**ため、
読めなくなると情報が失われる。事実の伝達と値の可読性は別問題である。
期限切れの日時には `--gray-500`（4.83:1）を使い、通常行の `--gray-700`（8.18:1）より
淡くすることで区別する。
...
本設計では日時の 2 セルだけ `--gray-500` にし、タイトルと説明は通常色のまま残す。
`--gray-400` は白背景で 2.08:1 で日時が読めなくなるため使わない。
